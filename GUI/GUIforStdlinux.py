...
is_connected = False
is_running = False
# Change the test script name here
...
def transfer_stl():
    ...
    # Change the .stl file path here. You can manually create an empty .STL file named output.stl in your directory
    sftp_client.get('/home/??????????????????????????/output.stl', 'output.stl')
    sftp_client.close()
